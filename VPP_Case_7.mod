...
param CHP_EtoT_F;  #Thermal to electric power ratio of CHP in the facilities (typical range is 2 to 10).
param CHP_EtoT_W;  #Thermal to electric power ratio of CHP in the facilities (typical range is 2 to 10).
param CHP_EtoT_S;  #Thermal to electric power ratio of CHP in the facilities (typical range is 2 to 10).
...
param D_E_F{f in Factory};        #Electric power load of factory. (unit: MW)
param D_E_W{w in Warehouse};
param D_E_S{s in Store}; 

param D_T_F{f in Factory};        #Thermal load of factory. (unit: MW)
param D_T_W{w in Warehouse};
param D_T_S{s in Store}; 
...
var ESS_E_F{t in steps,f in Factory}>=0,<=20;      #Electricity stored  in factory at t 
var ESS_E_W{t in steps,w in Warehouse}>=0,<=20;   #Electricity  stored   in warehouse at t
var ESS_E_S{t in steps,s in Store}>=0,<=20;        #Electricity  stored   in STORE at t
...
#Capacity cost
var cap_cost_Gens=sum{n in Gens,f in Factory}Q_GEN*a1_G[n]*G_F[n,f]
                  +sum{n in Gens,w in Warehouse}Q_GEN*a1_G[n]*G_W[n,w]
                  +sum{n in Gens,s in Store}Q_GEN*a1_G[n]*G_S[n,s];

var cap_cost_ESS=sum{f in Factory}Q_ESS*a1_ESS*ESS_F[f]
                  +sum{w in Warehouse}Q_ESS*a1_ESS*ESS_W[w]
                  +sum{s in Store}Q_ESS*a1_ESS*ESS_S[s];
...
#constraint
#meet demand in facto   #expand check the constraint
subject to energy_balance_factory{f in Factory, t in 1..T}:
sum{n in Gens}(G_F["WT",f]*CF_F[t,f,"WT"]+G_F["PV",f]*CF_F[t,f,"PV"])+ESS_E_F[t,f]-ESS_E_F[t-1,f]+CHP_E_F[t,f]>=D_E_F[f]+CHP_trade_F[t,f];

subject to energy_balance_warehouse{w in Warehouse, t in 1..T}:
sum{n in Gens}(G_W["WT",w]*CF_W[t,w,"WT"]+G_W["PV",w]*CF_W[t,w,"PV"])+ESS_E_W[t,w]-ESS_E_W[t-1,w]+CHP_E_W[t,w]>=D_E_W[w]+CHP_trade_W[t,w];

subject to energy_balance_Store{s in Store, t in 1..T}:
sum{n in Gens}(G_S["WT",s]*CF_S[t,s,"WT"]+G_S["PV",s]*CF_S[t,s,"PV"])+ESS_E_S[t,s]-ESS_E_S[t-1,s]+CHP_E_S[t,s]>=D_E_S[s]+CHP_trade_S[t,s];


subject to thermal_balance_factory{f in Factory, t in 1..T}:
        ESS_T_F[t,f]-ESS_T_F[t-1,f]+CHP_T_F[t,f]=D_T_F[f];

subject to thermal_balance_warehouse{w in Warehouse, t in 1..T}:
        ESS_T_W[t,w]-ESS_T_W[t-1,w]+CHP_T_W[t,w]=D_T_W[w];

subject to thermal_balance_Store{s in Store, t in 1..T}:
        ESS_T_S[t,s]-ESS_T_S[t-1,s]+CHP_T_S[t,s]=D_T_S[s];
